    ...
    pageEncoding="UTF-8"%>
        ...
        <body>
            <div class="col-sm-9">
                ...
                <form action="<c:url value='/car-park-car-list'/>" id="formSubmit" method="get">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <tbody>
                                        <c:forEach var="item" items="${model.listResult}">
                                            <tr>
                                                <td><input type="checkbox" id="checkbox_${item.carId}" value="${item.carId}"></td>
                                                <td>${item.licensePlate}</td>
                                                <td>${item.carType}</td>
                                                <td>${item.carColor}</td>
                                                <td>${item.company}</td>
                                                <c:forEach var="parking" items="${parkingLots}">
                                                    <c:if test="${item.parkId == parking.parkId}">
                                                        <td>${parking.parkName}</td>
                                                    </c:if>
                                                </c:forEach>
                                                <td>
                                                    <c:url var="editURL" value="/car-park-car-list">
                                                        <c:param name="type" value="edit" />
                                                        <c:param name="carId" value="${item.carId}" />
                                                    </c:url>
